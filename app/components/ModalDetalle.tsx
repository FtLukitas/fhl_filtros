...
export default function ModalDetalle({ filtro, onCerrar }: ModalDetalleProps) {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-lg shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[90vh] animate-in zoom-in-95 relative"
      ...
      >
        ...
        <div className="bg-blue-900 p-6 flex justify-between items-center text-white">
          <div>
            ...
            <h3 id="modal-filtro-titulo" className="text-3xl font-black">{filtro.codigo_fhl}</h3>
          </div>
          <div className="flex items-center gap-6">
            <div className="flex flex-col items-center">
              <button
                onClick={compartirFiltro}
                aria-label="Compartir enlace directo de este filtro"
                className="text-white hover:text-green-300 w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/10 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
                title="Compartir"
              >
                {copiado ? (
            ...
            <div className="flex flex-col items-center">
              <button
                onClick={onCerrar}
                className="text-white hover:text-red-400 font-black text-3xl w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/10 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
            ...
            {listaImagenes.length > 0 ? (
              <div className="flex flex-col gap-3">
                ...
                <div 
                  role="button"
                  tabIndex={0}
                  aria-label="Hacé click o presioná Enter para ampliar la imagen"
                  className="w-full bg-slate-50 border border-slate-200 rounded-lg p-2 md:p-4 flex items-center justify-center h-[280px] md:h-[380px] overflow-hidden shadow-inner transition-all cursor-zoom-in hover:border-blue-300 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-600"
                ...
                >
                  <img
                    ...
                    alt={`Filtro FHL ${filtro.codigo_fhl} - Imagen ${indiceImagen + 1} de ${listaImagenes.length}`}
                ...
                {zoomActivo && (
                  <div 
                    role="dialog"
                    aria-modal="true"
                    aria-label="Vista ampliada de la imagen del filtro"
                  ...
                  >
                    <img
                      src={listaImagenes[indiceImagen]}
                      alt={`Vista ampliada del filtro FHL ${filtro.codigo_fhl}`}
                ...
                {listaImagenes.length > 1 && (
                  <div className="flex gap-3 overflow-x-auto py-2 justify-center scrollbar-hide" role="tablist" aria-label="Miniaturas de producto">
                    {listaImagenes.map((url: string, idx: number) => (
                      <button
                        key={idx}
                        role="tab"
                        aria-selected={indiceImagen === idx}
                        aria-label={`Ver imagen ${idx + 1} de ${listaImagenes.length}`}
                        onClick={() => setIndiceImagen(idx)}
                        className={`w-20 h-16 md:w-24 md:h-20 rounded-md border-2 flex-shrink-0 overflow-hidden bg-slate-50 transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 ${
                          indiceImagen === idx 
              ...
              <div className="w-full bg-slate-50 border border-slate-200 rounded-lg p-6 flex items-center justify-center h-[260px] shadow-inner">

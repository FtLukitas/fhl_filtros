...
export default function TablaItems({ items, onActualizarItem, onEliminarItem }: TablaItemsProps) {
  ...
  if (items.length === 0) {
    return (
      <div className="bg-white rounded shadow-sm border border-slate-200 p-8 text-center">
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              ...
              <th className="text-center px-4 py-3 text-xs font-bold text-slate-600 uppercase tracking-wider w-28">
                Cantidad
              </th>
              <th className="text-right px-4 py-3 text-xs font-bold text-slate-600 uppercase tracking-wider w-36">
                Precio Unit.
              </th>
              <th className="text-right px-4 py-3 text-xs font-bold text-slate-600 uppercase tracking-wider w-32">
                Subtotal
              </th>
                  ...
                  <td className="px-4 py-3 text-center">
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (val === '') return; // permitir borrar antes de escribir nuevo valor
                        onActualizarItem(item.id, 'cantidad', Math.max(1, parseInt(val) || 1));
                      }}
                      className="w-20 text-center border border-slate-300 rounded px-2 py-1.5 text-sm text-slate-900 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    />
                  </td>
                  <td className="px-4 py-3 text-right">
                    <div className="inline-flex items-center">
                      <span className="text-slate-500 mr-1 font-medium">$</span>
                      <input
                        type="number"
                        min="0"
                        step="0.01"
                        ...
                        placeholder="0.00"
                        className="w-24 text-right border border-slate-300 rounded px-2 py-1.5 text-sm text-slate-900 font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                      />
                    </div>
                  </td>
